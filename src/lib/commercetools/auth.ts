...
export async function generateAnonymousAuthToken() {
...
export async function generateCustomerAuthToken(
  ...
  const tokenProvider = new TokenProvider({ sdkAuth }, tokenFlow);
  const authToken = (await tokenProvider.getTokenInfo()) as AuthToken;
...
export async function refreshAuthToken(expiredAuthToken: AuthToken) {
